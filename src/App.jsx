...
import { doc, setDoc, getDocs, onSnapshot, collection, addDoc, query, orderBy, getDoc, updateDoc } from 'firebase/firestore';
...
function Stock({products,updateProduct,addProduct,deleteProduct,photos,onPhoto,role,settings}){
  ...
  const submit=()=>{
    ...
    addProduct({...np,qty:Number(np.qty)||0,price:Number(np.price),cost:Number(np.cost)||38,reserved:0,region:np.region||'both'});
    ...
    <div>
      ...
      {addMode&&(
        <div style={{...S.card,borderColor:GOLD+'55',marginBottom:16}}>
          ...
          <div style={{marginBottom:10}}>
            <div style={S.label}>Регион продажи</div>
            <select style={S.select} value={np.region||'both'} onChange={e=>setNp(p=>({...p,region:e.target.value}))}>
              <option value="both">🌍 Везде (Душанбе + Россия)</option>
              <option value="dushanbe">🇹🇯 Только Душанбе</option>
              <option value="russia">🇷🇺 Только Россия</option>
            </select>
          </div>
...
function History({sales,onBack,deleteSale}){
  ...
  // Generate last 24 months + any months found in sales data
  const genMonths=()=>{
  ...
  const filtered=sales
    .filter(s=>month==='all'||s.date.startsWith(month))
    .filter(s=>filter==='all'||s.paymentMethod===filter);
    ...
    <div>
      ...
      <div style={{...S.row,marginBottom:12}}>
        <select style={{...S.select,flex:2}} value={month} onChange={e=>setMonth(e.target.value)}>
          ...
          {months.map(m=><option key={m} value={m}>{new Date(m+'-01').toLocaleDateString('ru-RU',{month:'long',year:'numeric'})}</option>)}
        </select>
...
function More({sales,expenses,products,monthRevenue,monthCOGS,monthExpenses,monthProfit,annaShare,settings,setSettings,onLogout,orders,setOrders,customers,addCustomer,purchases,addPurchase,updatePurchase,debts,addDebt,updateDebt,photos,returns,addReturn,shopOrders=[],shopCustomers=[],updateShopOrder,deleteSale,deleteOrder,deleteCustomer,deletePurchase,deleteDebt,deleteReturn,currentEmployee}){
  ...
  if(view==='siteanalytics') return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><SiteAnalytics/></div>;
  ...
  if(view==='customers')return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><Customers customers={customers} addCustomer={addCustomer} deleteCustomer={deleteCustomer} orders={orders} shopCustomers={shopCustomers} shopOrders={shopOrders}/></div>;
  ...
  const items=[
    ...
    {id:'siteanalytics',icon:'📈',label:'Аналитика сайта', sub:'Воронка продаж · Топ товаров'},
...
// ── КЛИЕНТЫ ───────────────────────────────────────────────────
function Customers({customers,addCustomer,deleteCustomer,orders,shopCustomers=[],shopOrders=[]}){
  const [show,setShow]=useState(false);
  const [f,setF]=useState({name:'',phone:'',note:''});
  const [msg,setMsg]=useState('');
  const [filter,setFilter]=useState('all'); // all | manual | site
  ...
  // Merge manual customers with site customers by phone, so the same person
  // doesn't appear twice if they exist in both lists.
  const normPhone=(p)=>String(p||'').replace(/\D/g,'');
  const manualByPhone={};
  customers.forEach(c=>{ if(c.phone) manualByPhone[normPhone(c.phone)]=true; });

  const siteOnly=shopCustomers.filter(sc=>!manualByPhone[normPhone(sc.phone)]);

  const merged=[
    ...customers.map(c=>({...c, source:'manual'})),
    ...siteOnly.map(c=>({...c, source:'site'})),
  ];

  const enriched=merged.map(c=>{
    const phoneN=normPhone(c.phone);
    // Count both manually-tracked orders AND real website orders for this person
    const manualOrders=orders.filter(o=>(o.customerPhone&&normPhone(o.customerPhone)===phoneN)||o.customerName===c.name);
    const siteOrders=shopOrders.filter(o=>o.customerPhone&&normPhone(o.customerPhone)===phoneN);
    const allOrders=[...manualOrders, ...siteOrders];
    return{
      ...c,
      orderCount:allOrders.length,
      totalSpent:allOrders.filter(o=>o.status==='done').reduce((s,o)=>s+o.total,0),
    };
  }).sort((a,b)=>b.totalSpent-a.totalSpent);

  const filtered = filter==='all' ? enriched : enriched.filter(c=>c.source===filter);
  const siteCount = enriched.filter(c=>c.source==='site').length;
  const manualCount = enriched.filter(c=>c.source==='manual').length;

  return(
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
        <div style={S.sec}>Клиенты ({enriched.length})</div>
        <button style={S.btnSm(show?RED:GOLD)} onClick={()=>setShow(!show)}>{show?'✕':'+ Клиент'}</button>
      </div>

      <div style={{display:'flex',gap:6,marginBottom:12,overflowX:'auto'}}>
        <button onClick={()=>setFilter('all')} style={{...S.btnSm(filter==='all'?GOLD:MUTED),fontSize:11,padding:'6px 10px'}}>Все ({enriched.length})</button>
        <button onClick={()=>setFilter('site')} style={{...S.btnSm(filter==='site'?BLUE:MUTED),fontSize:11,padding:'6px 10px'}}>🌐 С сайта ({siteCount})</button>
        <button onClick={()=>setFilter('manual')} style={{...S.btnSm(filter==='manual'?GREEN:MUTED),fontSize:11,padding:'6px 10px'}}>✏️ Вручную ({manualCount})</button>
      </div>
      ...
      {filtered.length===0&&<div style={{color:MUTED,textAlign:'center',padding:'24px 0',fontSize:13}}>Клиентов пока нет</div>}
      {filtered.map(c=>(
        <div key={c.source+'_'+(c.id||c.phone)} style={S.card}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            <div style={{flex:1}}>
              <div style={{display:'flex',alignItems:'center',gap:6}}>
                <div style={{fontSize:15,fontWeight:700}}>{c.name}</div>
                {c.source==='site'&&<Badge text="С сайта" color={BLUE}/>}
              </div>
            ...
            <div style={{textAlign:'right'}}>
              ...
              {c.source==='manual'&&<button onClick={()=>deleteCustomer(c.id)} style={{background:'none',border:'none',color:RED,fontSize:14,cursor:'pointer',marginTop:4}}>🗑 Удалить</button>}
            </div>
          </div>
        </div>
      ))}
...
// ── АНАЛИТИКА САЙТА (воронка продаж) ─────────────────────────
function SiteAnalytics(){
  ...
  useEffect(()=>{
    ...
    getDocs(query(collection(db,'analytics'))).then(snap=>{
      const all = snap.docs.map(d=>d.data()).filter(d=>d.date>=since);
      const counts = {
        page_view:    all.filter(d=>d.event==='page_view').length,
        product_view: all.filter(d=>d.event==='product_view').length,
        add_to_cart:  all.filter(d=>d.event==='add_to_cart').length,
        checkout_start: all.filter(d=>d.event==='checkout_start').length,
        order_placed: all.filter(d=>d.event==='order_placed').length,
      };
      // Top products
      const pv = all.filter(d=>d.event==='product_view');
      const pvCount = {};
      pv.forEach(d=>{ pvCount[d.productName]=(pvCount[d.productName]||0)+1; });
      const topProducts = Object.entries(pvCount).sort((a,b)=>b[1]-a[1]).slice(0,5);
      // Mobile vs desktop
      const mobile = all.filter(d=>d.ua==='mobile').length;
      setData({...counts, topProducts, mobile, total:all.length});
      setLoading(false);
    }).catch(()=>setLoading(false));
  },[days]);

  const funnel = [
    {label:'Зашли на сайт',  count:data.page_view||0,    pct:100},
    {label:'Смотрели товар', count:data.product_view||0, pct:data.page_view?Math.round(data.product_view/data.page_view*100):0},
    {label:'В корзину',      count:data.add_to_cart||0,  pct:data.page_view?Math.round(data.add_to_cart/data.page_view*100):0},
    {label:'Оформили',       count:data.checkout_start||0,pct:data.page_view?Math.round(data.checkout_start/data.page_view*100):0},
    {label:'Заказали',       count:data.order_placed||0, pct:data.page_view?Math.round(data.order_placed/data.page_view*100):0},
  ];
  ...
  return(
    <div>
      <div style={{display:'flex',gap:8,marginBottom:14}}>
        {[7,14,30].map(d=>(
          <button key={d} style={S.btnSm(days===d?GOLD:MUTED)} onClick={()=>setDays(d)}>{d} дней</button>
        ))}
      </div>

      <div style={{...S.card,marginBottom:12}}>
        <div style={S.sec}>📱 Устройства</div>
        <FR label="С телефона" value={`${data.mobile||0} из ${data.total||0}`}/>
        <FR label="С компьютера" value={`${(data.total||0)-(data.mobile||0)}`}/>
      </div>

      <div style={{...S.card,marginBottom:12}}>
        <div style={S.sec}>📊 Воронка продаж</div>
        {funnel.map((f,i)=>(
          <div key={i} style={{marginBottom:10}}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
              <div style={{fontSize:13,color:INK||DARK}}>{f.label}</div>
              <div style={{fontSize:13,fontWeight:700,color:GOLD}}>{f.count} ({f.pct}%)</div>
            </div>
            <div style={{height:6,background:'#222',borderRadius:3,overflow:'hidden'}}>
              <div style={{height:'100%',width:`${f.pct}%`,background:GOLD,borderRadius:3}}/>
            </div>
          </div>
        ))}
        {(data.page_view||0)===0&&<div style={{color:MUTED,fontSize:13,textAlign:'center',padding:12}}>Данных пока нет — они появятся когда покупатели зайдут на сайт</div>}
      </div>

      {(data.topProducts||[]).length>0&&(
        <div style={S.card}>
          <div style={S.sec}>🔥 Топ товаров по просмотрам</div>
          {(data.topProducts||[]).map(([name,count],i)=>(
            <div key={i} style={{...S.li}}>
              <div style={{fontSize:13}}>{i+1}. {name}</div>
              <Badge text={`${count} просм.`} color={GOLD}/>
            </div>
          ))}
        </div>
      )}
...
export default function QueenStar(){
  ...
  useEffect(() => {
    ...
    // Listen to customers who registered on the shop website
    try {
      const shopCustUnsub = onSnapshot(collection(db,'shop_customers'), snap => {
        setShopCustomers(snap.docs.map(d => ({id:d.id,...d.data(),fromShop:true})));
      });
      unsubs.push(shopCustUnsub);
    } catch {}
