...
const INIT_SETTINGS={usdRate:10.9,cnyRate:1.5,cargoUsd:2.6,markup:3,pin:'1234',lowStock:5,telegramToken:'',telegramChatId:'',monthlyGoal:15000};
...
function More({sales,expenses,products,monthRevenue,monthCOGS,monthExpenses,monthProfit,annaShare,settings,setSettings,onLogout,orders,setOrders,customers,addCustomer,purchases,addPurchase,updatePurchase,debts,addDebt,updateDebt,photos,returns,addReturn,shopOrders=[]}){
  ...
  if(view==='returns')  return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><Returns returns={returns} addReturn={addReturn} products={products}/></div>;
  ...
  if(view==='orders')   return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><Orders orders={orders} setOrders={setOrders} products={products} shopOrders={shopOrders}/></div>;
...
// ── ЗАКАЗЫ ────────────────────────────────────────────────────
function Orders({orders,setOrders,products,shopOrders=[]}){
  ...
  return(
    <div>
      ...
      {shopOrders.filter(o=>o.status==='new').length>0&&(
        <div style={{...{...S.card},borderColor:BLUE+'55',marginBottom:12}}>
          <div style={{...S.sec,color:BLUE}}>🌐 С сайта ({shopOrders.filter(o=>o.status==='new').length})</div>
          {shopOrders.filter(o=>o.status==='new').map(o=>(
...
// ── ВОЗВРАТЫ ─────────────────────────────────────────────────
function Returns({returns,addReturn,products}){
  const [show,setShow]=useState(false);
...
export default function QueenStar(){
  ...
  const saveFS = (key, data) => {
    ...
    if (saving.current[key] === v) return; // skip if same data
    saving.current[key] = v;
    setDoc(doc(db, 'qs', key), { v, ts: Date.now() }).catch(e => console.error('Save error:', key, e));
  };
      ...
      try {
        return onSnapshot(
          ...
          snap => {
            ...
            if (v === saving.current[key]) return;
            try { setter(JSON.parse(v)); saving.current[key] = v; } catch {}
          },
  ...
  useEffect(()=>{if(ready)saveFS('suppliers',suppliers);},[suppliers,ready]);
  useEffect(()=>{if(ready)saveFS('promotions',promotions);},[promotions,ready]);
  ...
  const addReturn=(r)=>{
    ...
    setProducts(prev=>prev.map(p=>{
      ...
      const newQty=p.qty+Number(r.qty);
      if(r.size&&p.sizes){
        const newSizes=(p.sizes||[]).map(s=>s.size===r.size?{...s,qty:s.qty+Number(r.qty)}:s);
        return{...p,qty:newQty,sizes:newSizes};
      }
      return{...p,qty:newQty};
    }));
  };
  const addSupplier=(s)=>setSuppliers(prev=>[{id:Date.now(),date:nowStr(),...s},...prev]);
  const updateSupplier=(id,f,v)=>setSuppliers(prev=>prev.map(s=>s.id===id?{...s,[f]:v}:s));
  const addPromotion=(p)=>setPromotions(prev=>[{id:Date.now(),active:true,...p},...prev]);
  const updatePromotion=(id,f,v)=>setPromotions(prev=>prev.map(p=>p.id===id?{...p,[f]:v}:p));
