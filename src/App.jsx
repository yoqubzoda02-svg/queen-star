...
function Dashboard({todayRevenue,totalStock,monthProfit,annaShare,todaySales,products,settings,sales}){
...
function More({sales,expenses,products,monthRevenue,monthCOGS,monthExpenses,monthProfit,annaShare,settings,setSettings,onLogout,orders,setOrders,customers,addCustomer,purchases,addPurchase,updatePurchase,debts,addDebt,updateDebt,photos,returns,addReturn,shopOrders=[],updateShopOrder,deleteSale,deleteOrder,deleteCustomer,deletePurchase,deleteDebt,deleteReturn,currentEmployee}){
  ...
  if(view==='shift')    return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><ShiftManager sales={sales} currentEmployee={currentEmployee} db={db}/></div>;
...
function ShiftManager({sales,currentEmployee,db}){
...
export default function QueenStar(){
  ...
  useEffect(()=>{
    ...
    products.filter(p=>!p.transit&&(p.qty-(p.reserved||0))<=(settingsRef.current.lowStock||5)&&p.qty>0)
      .forEach(p=>{
        if(!window._qs_low)window._qs_low={};
        const k=p.id+'_'+p.qty;
        if(!window._qs_low[k]){window._qs_low[k]=1;sendTelegram('⚠️ Заканчивается: '+p.name+' — осталось '+(p.qty-(p.reserved||0))+' пар!');}
      });
  },[products,ready]);
  ...
  if(!role)return(
    ...
    <div style={{background:BG,minHeight:'100vh',color:CREAM,fontFamily:"'DM Sans',sans-serif",paddingBottom:76}}>
      ...
      <div style={S.page}>
        {tab==='dash'&&<Dashboard {...{todayRevenue,totalStock,todaySales,products,settings,sales,...fin}}/>}
