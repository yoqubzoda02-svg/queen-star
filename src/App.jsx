...
function EmployeePhoneLogin({onLogin}){
  ...
  const sendSMS=async()=>{
    ...
    try{
      ...
      setErr(e.code==='auth/invalid-phone-number'?'Неверный формат номера':'Ошибка отправки SMS');
      rcRef.current=null;
    }
    setLoading(false);
  };
...
function LoginScreen({onLogin,ownerPin}){
  ...
  const [pin,setPin]=useState('');
  const [err,setErr]=useState('');
  const try_=()=>{if(pin===ownerPin)onLogin('owner',null);else{setErr('Неверный PIN');setPin('');}};
  if(showEmp)return(
    <div style={{background:BG,minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:28}}>
      <div style={{fontFamily:"'Playfair Display',serif",fontSize:24,color:GOLD,letterSpacing:3,marginBottom:40}}>QUEEN STAR</div>
      <EmployeeLogin onLogin={(emp)=>onLogin('employee',emp)}/>
      <button style={{...S.btnSm(MUTED),marginTop:24}} onClick={()=>setShowEmp(false)}>← Назад</button>
    </div>
  );
  return(
    <div style={{background:BG,minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:28}}>
      ...
      {!showPin?(
        <>
          ...
          <button style={{...S.btn('#2A2420'),borderRadius:12,padding:'16px 24px',fontSize:15,color:CREAM}} onClick={()=>setShowEmp(true)}>🏪 Сотрудник</button>
          <div style={{fontSize:11,color:MUTED,marginTop:24,textAlign:'center',lineHeight:1.8}}>Сотрудник: вход по SMS<br/>Владелец: PIN-код</div>
        </>
      ):(
        <div style={{width:'100%',maxWidth:280}}>
...
function More({sales,expenses,products,monthRevenue,monthCOGS,monthExpenses,monthProfit,annaShare,settings,setSettings,onLogout,orders,setOrders,customers,addCustomer,purchases,addPurchase,updatePurchase,debts,addDebt,updateDebt,shifts}){
...
export default function QueenStar(){
  ...
  useEffect(()=>{
    (async()=>{
      try{
        const keys=['products','sales','expenses','settings','photos','orders','customers','purchases','debts','shifts'];
        const defs=[INIT_PRODUCTS,[],[],INIT_SETTINGS,{},[],[],[],[],[]];
        const sets=[setProducts,setSales,setExpenses,setSettings,setPhotos,setOrders,setCustomers,setPurchases,setDebts,setShifts];
        try{
          const snaps=await Promise.allSettled(keys.map(k=>getDoc(doc(db,'qs',k))));
          snaps.forEach((s,i)=>{if(s.status==='fulfilled'&&s.value.exists())try{sets[i](JSON.parse(s.value.data().v));}catch{}});
  ...
  useEffect(()=>{
    const listen=(key,setter)=>onSnapshot(doc(db,'qs',key),snap=>{
      if(snap.exists()){const{v,ts}=snap.data();if(ts!==lw.current[key])try{setter(JSON.parse(v));}catch{}}
    });
    const unsubs=[listen('products',setProducts),listen('sales',setSales),listen('expenses',setExpenses),listen('settings',setSettings),listen('orders',setOrders),listen('customers',setCustomers),listen('purchases',setPurchases),listen('debts',setDebts),listen('shifts',setShifts)];
  ...
  useEffect(()=>{if(ready)saveFS('settings',settings);},[settings,ready]);
  ...
  return(
    <div style={{background:BG,minHeight:'100vh',color:CREAM,fontFamily:"'DM Sans',sans-serif",paddingBottom:76}}>
      ...
      <div style={S.page}>
        ...
        {tab==='more'&&role==='owner'&&<More {...{sales,expenses,...shared,...fin,settings,setSettings,onLogout:()=>setRole(null),orders,setOrders,customers,setCustomers,purchases,setPurchases,debts,setDebts,addOrder,addCustomer,addPurchase,addDebt,updateOrder,updateCustomer,updatePurchase,updateDebt}}/>}
      </div>
