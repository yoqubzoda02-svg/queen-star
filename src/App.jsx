  ...
  return(
    <div style={{...S.card,borderColor:isLow?AMBER+'55':BORD,opacity:transit?0.72:1}}>
      ...
      {!transit&&!editing&&(
        ...
        <div style={{marginTop:8}}><QRCode productId={product.id} productName={product.name} price={product.price}/></div>
      ...
      <div style={S.card}>
        ...
        {rates && (
          <button style={{ ...S.btn(GOLD), marginTop:12 }}
            onClick={()=>setSettings(p=>({...p, usdRate:rates.usd, cnyRate:rates.cny}))}>
...
export default function QueenStar(){
      ...
      try{
        ...
        snaps.forEach((s,i)=>{
          if(s.status==='fulfilled'&&s.value?.exists&&s.value.exists())
            try{const v=s.value.data()?.v;if(v)sets[i](JSON.parse(v));}catch{}
        });
  ...
  useEffect(()=>{
    fetch('https://open.er-api.com/v6/latest/USD')
      .then(r=>r.json())
      .then(d=>{if(d.rates?.TJS){const usd=parseFloat(d.rates.TJS.toFixed(2));setSettings(p=>({...p,usdRate:usd}));}})
      .catch(()=>{});
  },[]);
  ...
  return(
    <div style={{background:BG,minHeight:'100vh',color:CREAM,fontFamily:"'DM Sans',sans-serif",paddingBottom:76}}>
      ...
      <div style={S.page}>
        ...
        {tab==='more'&&role==='owner'&&<More {...{sales,expenses,...shared,...fin,settings,setSettings,photos,onLogout:()=>setRole(null),orders,setOrders,customers,setCustomers,purchases,setPurchases,debts,setDebts,addOrder,addCustomer,addPurchase,addDebt,updateOrder,updateCustomer,updatePurchase,updateDebt}}/>}
      </div>
