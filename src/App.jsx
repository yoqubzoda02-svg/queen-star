...
function More({sales,expenses,products,monthRevenue,monthCOGS,monthExpenses,monthProfit,annaShare,settings,setSettings,onLogout,orders,setOrders,customers,addCustomer,purchases,addPurchase,updatePurchase,debts,addDebt,updateDebt,photos,returns,addReturn,shopOrders=[],deleteSale,deleteOrder,deleteCustomer,deletePurchase,deleteDebt,deleteReturn}){
  ...
  if(view==='orders')   return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><Orders orders={orders} setOrders={setOrders} products={products} shopOrders={shopOrders} deleteOrder={deleteOrder}/></div>;
...
// ── ЗАКАЗЫ ────────────────────────────────────────────────────
function Orders({orders,setOrders,products,shopOrders=[],deleteOrder}){
