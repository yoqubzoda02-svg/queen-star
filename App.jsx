...
export default function QueenStar(){
  ...
  useEffect(()=>{
    (async()=>{
      try{
        const keys=['qs_products','qs_sales','qs_expenses','qs_settings','qs_photos'];
        const res=await Promise.allSettled(keys.map(k=>window.storage.get(k)));
        const ok=(r)=>r.status==='fulfilled'&&r.value?.value;
        if(ok(res[0]))setProducts(JSON.parse(res[0].value.value));
        if(ok(res[1]))setSales(JSON.parse(res[1].value.value));
        if(ok(res[2]))setExpenses(JSON.parse(res[2].value.value));
        if(ok(res[3]))setSettings(JSON.parse(res[3].value.value));
        if(ok(res[4]))setPhotos(JSON.parse(res[4].value.value));
      }catch{}
      setReady(true);
    })();
  },[]);

  useEffect(()=>{if(ready)window.storage.set('qs_products',JSON.stringify(products)).catch(()=>{});},[products,ready]);
  useEffect(()=>{if(ready)window.storage.set('qs_sales',JSON.stringify(sales)).catch(()=>{});},[sales,ready]);
  useEffect(()=>{if(ready)window.storage.set('qs_expenses',JSON.stringify(expenses)).catch(()=>{});},[expenses,ready]);
  useEffect(()=>{if(ready)window.storage.set('qs_settings',JSON.stringify(settings)).catch(()=>{});},[settings,ready]);
  useEffect(()=>{if(ready)window.storage.set('qs_photos',JSON.stringify(photos)).catch(()=>{});},[photos,ready]);
